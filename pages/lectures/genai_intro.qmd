...
Generative models are often introduced as models that create images from text prompts. However, for computer vision this is only one important use case. More generally, generative models produce or modify visual data under different types of constraints.

A useful distinction is between **generation from scratch**, **editing existing data**, and **generating missing or enhanced information**.



| Task | Input | Output | Example |
|------|-------|--------|---------|
| **Unconditional image generation** | random noise | image | random face, random bedroom, random product |
| **Class-conditional generation** | class label + noise | image | “dog”, “car”, “elephant” |
| **Text-to-image generation** | text prompt + noise | image | “a red chair in a modern living room” |
| **Image-to-image generation** | image + instruction / prompt | modified image | change season, style, material, lighting |
| **Inpainting** | image + mask + prompt | completed image | remove object, fill missing region |
| **Outpainting** | image + extended canvas | expanded image | extend background beyond image borders |
| **Super-resolution / enhancement** | low-resolution image | high-resolution image | upscaling, denoising, sharpening |
| **Image translation** | source image | target-domain image | MRI to CT, sketch to photo, day to night |
| **Synthetic data generation** | task specification | training examples | rare classes, simulated defects, edge cases |
| **Video generation** | prompt or image sequence | video | short clips, object motion, scene animation |

The important point is that generative computer vision is not only about creating new images. Many practical applications are **conditional**: the model must preserve some information from the input while generating other parts.

::: {.callout-note title="Discussion"}
Which of these tasks require the model to be creative, and which require the model to be faithful to a given input?
:::
...
A central distinction in generative modelling is whether the model generates samples freely or under additional constraints.

In **unconditional generation**, the model receives only a random input, typically sampled from a simple distribution:
...
Here, $\vect{z}$ controls random variation. For example, sampling different values of $\vect{z}$ may generate different faces, animals, or rooms.

In **conditional generation**, the model additionally receives some conditioning information $\vect{c}$:

$$
\hat{\vect{x}} = g_{\theta}(\vect{z}, \vect{c})
$$

The condition $\vect{c}$ specifies what should be generated or preserved.

| Type of condition $\vect{c}$ | Example task |
|------------------------------|--------------|
| class label | generate an image of an elephant |
| text prompt | generate “a small cabin next to a lake” |
| input image | edit this image |
| mask | fill this missing region |
| low-resolution image | super-resolution |
| segmentation map | generate an image with this layout |
| pose / depth / edge map | control geometry and structure |
| reference image | preserve identity, product, or style |
...
Unconditional vs. conditional generation. The top row shows random samples of people, the bottom row shows samples of people conditioned on "create three images of young people with glasses and curly hair."
:::

Conditional generation is especially important in computer vision because many applications require **control**. We usually do not just want any plausible image; we want an image that matches a prompt, preserves a product, fills a masked region, follows a layout, or remains consistent with a source image.

::: {.callout-note title="Key idea"}
The random variable $\vect{z}$ provides **diversity**, while the condition $\vect{c}$ provides **control**.
:::
