...
The integration of deep learning in computer vision has massively improved the accuracy and efficiency of visual recognition, classification, and analysis. This has opened up new possibilities in applications such as autonomous driving, facial recognition, and medical image analysis. These models are now embedded in a wide variety of products and services.
...
Deep learning can be used to identify objects, to find similar images (image search / retrieval), or to find (near-)duplicates (perceptual hashing), see @fig-intro-google-lens (left). Increasingly, models can cross modalities and, for example, process visual data and produce textual outputs. An interesting example is image captioning or the detection and translation of text in images  (optical character recognition - OCR), see @fig-intro-google-lens (right).
...
Object detection involves locating and recognizing multiple objects in an image. @fig-intro-yolo-example shows an example @Redmon2016a. Each object is localized with a bounding box and assigned an object class with a confidence score.
...
In keypoint detection, specific points on objects — typically human body joints — are localized. @fig-mask-rcnn-keypoint-detection-example shows an example from the paper by @he_mask_2018. This can be used to transfer movements from a person to an avatar or to recognize human activities (action recognition).

::: {#fig-mask-rcnn-keypoint-detection-example}

![]({{< meta params.images_path >}}keypoint_detection_exampe_maskrcnn.jpg){width=600}
...
An image is not the world. It is a 2D projection shaped by viewpoint, lighting, materials, motion, and occlusion. Computer vision is the problem of inferring useful structure from that incomplete signal. Understanding the challenges of vision starts with recognizing this: pixels are measurements, not meanings (@torralba_foundations_2024).
...
@fig-blop-context represents an interesting example where context is important: Shown are four images with severe quality degradation (blur) and oval objects indicated with dashed black lines. Notably, each marked object is exactly identical on pixel level (apart from a rotation) — yet we can form clear hypotheses what each of the objects represents.
...
### Images as Physical Evidence

Beyond semantic labels, images carry cues about the *physical causes* that produced them — geometry, lighting, materials, and motion. A pixel value is not arbitrary; it results from light bouncing off surfaces, refracted through a lens, and measured by a sensor. Computer vision is partly the problem of inverting this physical process: recovering the 3D structure, surface properties, and motion from the 2D projection (@torralba_foundations_2024).

Consider @fig-intro-physics-cues: vanishing points reveal the geometry of a scene and the camera's viewpoint; dappled shadows act as a lens array, encoding 3D structure through circular spots; motion blur encodes velocity and exposure time; surface wetness changes reflectance and adds specular highlights. Each is a different kind of evidence about the hidden 3D world.

::: {#fig-intro-physics-cues}

![]({{< meta params.images_path >}}physics_cues_collage.png){width=700}

Images encode geometry, optics, and material properties. **Top left:** vanishing points define depth and perspective. **Top right:** pinhole shadows are circles, independent of leaf shape. **Bottom left:** motion blur captures velocity and exposure. **Bottom right:** surface wetness changes appearance through reflectance.
:::

This is why computer vision spans so many sub-fields: each addresses a different inference problem from image evidence. Understanding vision means understanding not just *what* is in a scene, but *why it looks the way it does*.
...
The key conceptual shift is from hand-designed feature pipelines to **end-to-end learning**. Rather than designing features manually, the model learns to extract relevant features directly from raw pixels. @fig-intro-end-to-end illustrates this paradigm.
