...
ImageNet-pretrained ResNets remain solid baselines, but a new generation of foundation-model backbones trained on orders-of-magnitude more data consistently outperforms them — often with less fine-tuning data required. The table below summarizes the most practical options as of 2025:

| Model | Pretraining | Strengths | Good for |
|-------|-------------|-----------|----------|
| **DINOv2** / **DINOv3** | Self-supervised (images) | Strong general-purpose features; no labels needed | Feature extraction, k-NN, linear probe |
| **SigLIP** | Vision–language (image–text) | Zero-shot and few-shot transfer; multilingual | Open-vocabulary classification, retrieval |
| **ConvNeXt-V2** | Self-supervised (FCMAE) | Pure CNN; no attention; fast inference | When ViT overhead is a concern |
| **SAM** / **SAM 2** | Large-scale segmentation | Powerful spatial features; click-based prompting | Segmentation, detection pre-training |
| **EVA** / **EVA-CLIP** | Vision–language at scale | State-of-the-art on many benchmarks | High-accuracy transfer learning |

: {tbl-colwidths="[18,25,30,27]"}

**Practical guidance:**

- **Default choice**: DINOv2 (ViT-B/14 or ViT-L/14) is a strong all-round starting point for feature extraction and fine-tuning on domain-specific data.
- **Small datasets** (< 1 000 labeled images): freeze the backbone and train only a linear head. DINOv2 and SigLIP features work well out-of-the-box.
- **Medium datasets** (1 000–50 000 images): fine-tune the top few transformer blocks while keeping early layers frozen.
- **Large datasets or domain shift** (e.g., satellite imagery, medical imaging): full fine-tuning from a pretrained checkpoint usually beats training from scratch.
- **Speed-constrained deployment**: ConvNeXt-V2 avoids the quadratic attention cost of ViTs and is easier to export to ONNX / TensorRT.
