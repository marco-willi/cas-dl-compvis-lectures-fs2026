...
::: {.callout-tip title="Downloading Snapshot Serengeti"}
...
```bash
curl -O https://marco-willi.github.io/cas-dl-compvis-lectures-fs2026/assets/datasets/download_ser.py
pip install pandas requests Pillow
python download_ser.py
```

Edit the configuration block at the top of the script to select species and image counts. The first run builds a local catalogue (~1 min); subsequent runs load the cache and go straight to downloading.
